...
#pragma clang diagnostic push
            ...
            [[UIApplication sharedApplication] setStatusBarStyle:(_statusBarStyle ? _statusBarStyle : UIStatusBarStyleDefault) animated:animated];
        }
    }
...
- (MWCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    ...
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        ...
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) {
                captionView = [MWCaptionView alloc];
                captionView.captionTintColor = _captionTintColor;
                captionView.captionBarTintColor = _captionBarTintColor;
                captionView.captionBackgroundColor = _captionBackgroundColor;
                captionView.captionTextColor = _captionTextColor;
                captionView.captionShadowColor = _captionShadowColor;
                captionView = [captionView initWithPhoto:photo];
            }
        }
    }
...
- (void)doneButtonPressed:(id)sender {
    // Only if we're modal and there's a close button and selected pictures in modal style
    if (_closeButton && _displayPicturesInModalViewWithGrid) {
        [self showCloseButton:NO];
    }
    // Only if we're modal and there's a done button
    if (_doneButton || _closeButton) {
...
- (void)closeButtonPreseed:(id)sender {
